...
- **Arithmetic:** `+ - *` keep type; **`int / int → rational` and `/` is `<decides>`** (fails on 0 divisor). `+` concatenates strings/arrays. `Mod[a,b]` and `Quotient[a,b]` are `<decides>` functions (call with `[]`). **Operands must be the SAME numeric type** — `Count / 10.0` with an int Count is a compile error (3509), not a conversion; write `(1.0 * Count) / 10.0`.
...
- **Operator overloading:** module-scope functions with quoted operator names, for your own types:
  ```verse
  operator'+'<public>(L:my_vec, R:my_vec)<computes>:my_vec = my_vec{X := L.X + R.X}
  operator'*'<public>(S:float, V:my_vec)<computes>:my_vec = ...  # asymmetric operands OK; add the flipped order as a second overload
  ```
  Overloadable: binary `+ - * /` and custom named infix `operator'name'(A, B)` (precedence of `*`). **NOT overloadable: `< <= > >= = <>` and `+= -= *= /=`.** Compound assigns don't desugar to your `operator'+'` — they resolve ONLY against the builtins (`operator'+='(:ref int,:int)`, `(:ref float,:float)`, `(:ref []t,:[]t)`, map insert). So `set Arr += X` needs `X:[]t` whose element type unifies with Arr's — if a subtype relation breaks (e.g. an element class stops implementing the array's interface type), you get "No overload of operator'+='" (3509), and the fix is retyping the variable, not writing an operator.
...
- **Parametric-type module layering (current-build bugs):** parametric definitions must live in a module **strictly above** all their generic users (3502 fires on same-level use, and sibling files don't order reliably) — the robust layout is ONE file with explicit `module:` nesting: interfaces at file-root, implementations in a child module, extensions in a grandchild module. A parametric class takes at most ONE parametric superinterface — combine several into one interface first (3502); don't nest a parametric class as a field type of another parametric class (3509 mis-unification — flatten to parallel arrays); avoid `var` fields whose type mentions `t` on classes that also get `t`-generic extensions (3552 — prefer member functions); zero-arg extensions leave `t` unused (3503 — prefer members); upcast elements explicitly when returning `[]derived(t)` as `[]base(t)` (3510). Full list + fixes in `gotchas.md`.
